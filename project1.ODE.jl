...
# ╔═╡ 292d2055-cf04-4043-80ee-085d71dfb6a5
using DifferentialEquations

# ╔═╡ 23a7fe30-1961-4496-8228-761bc1291d9c
using Plots

# ╔═╡ dac31fc8-3109-4e79-a4b6-c1ab2f238217
# Physical constants
gravity_g = 9.81                    # [m/s^2]

# ╔═╡ aa51831f-75cb-4089-96b2-8d93a2607f97
pendulum_length_L = 0.15            # [m]

# ╔═╡ 634747d8-9c92-42b8-a252-c34da98b4d7a
pendulum_mass_m = 0.10              # [kg]

# ╔═╡ 321359c0-5321-4886-bae9-9b43baef0331
frame_height_h1 = 0.20              # [m]

# ╔═╡ 74d9f36e-f6c9-45a8-bcf8-3d41b61c6c47
frame_offset_w1 = 0.10              # [m]

# ╔═╡ 4eb5732d-7acb-4f77-9733-5dcb5a809ef2
# Rotation rates [rad/s]
rotation_rate_slow = 2.0            # "slow" (below natural frequency ~√(g/L))

# ╔═╡ d35aff4f-f9e1-4e0e-9b38-1a8cbe4b5617
rotation_rate_fast = 20.0           # "fast"

# ╔═╡ 74d49c90-b97e-4ba8-8adb-e3cc7763e481
# uses DifferentialEquations.jl (assumed imported)
function spinning_pendulum_rhs!(state_derivative, state, parameters, time)
    angle_theta = state[1]
    angular_velocity_theta = state[2]

    rotation_rate_Omega = parameters.Omega
    pendulum_length_L   = parameters.L     # FIXED HERE
    frame_offset_w1     = parameters.w1    # FIXED HERE
    gravity_g           = parameters.g     # FIXED HERE

    state_derivative[1] = angular_velocity_theta
    state_derivative[2] =
        (rotation_rate_Omega^2 / pendulum_length_L) *
        (frame_offset_w1 + pendulum_length_L * sin(angle_theta)) * cos(angle_theta) -
        (gravity_g / pendulum_length_L) * sin(angle_theta)
end

# ╔═╡ 2571e0d4-41cf-4faf-934d-22b002a7f063
``

# ╔═╡ 9f903ba3-d708-4929-9013-446b8a1283c5
### Pluto cell: ODEProblem for slow rotation (fixed parameters)

initial_angle_deg = 10.0

# ╔═╡ 758dad16-6bcd-49e6-978f-94950a064d74
initial_angle_rad = initial_angle_deg * pi / 180.0

# ╔═╡ 61a1d235-35b6-4d4b-942f-f595351281f0
initial_state = [initial_angle_rad, 0.0]

# ╔═╡ 96a77224-e753-40e0-a423-fea813af0b75
time_span = (0.0, 10.0)

# ╔═╡ 3c541102-7a04-4f11-a4b1-ce00eb0681a2
parameters_slow = (
    Omega = rotation_rate_slow,
    L = pendulum_length_L,
    w1 = frame_offset_w1,
    g = gravity_g,
)

# ╔═╡ 5e681481-668a-42a0-94ff-63fdd074139c
parameters_fast = (
    Omega = rotation_rate_fast,
    L = pendulum_length_L,
    w1 = frame_offset_w1,
    g = gravity_g,
)

# ╔═╡ 44dbad9a-3c10-430d-a86a-9d20f13d69cb
problem_fast = ODEProblem(spinning_pendulum_rhs!, initial_state, time_span, parameters_fast)

# ╔═╡ ec0251ae-d975-4bc4-a28b-38c5497e3871
solution_fast = solve(problem_fast)

# ╔═╡ 0133f999-c65a-4c42-9ec9-a8a20da827d9
plot(solution_slow.t, getindex.(solution_slow.u, 1),
     label = "Slow rotation θ(t)",
     xlabel = "Time [s]", ylabel = "Angle θ [rad]", legend = :topright)

# ╔═╡ d8fe4d4c-6f57-4dc1-a257-c6be3e8e3779
plot!(solution_fast.t, getindex.(solution_fast.u, 1),
      label = "Fast rotation θ(t)")

# ╔═╡ 58d8f9b4-85d0-4bc5-b104-40761870d351
number_of_frames = length(solution_slow.t)

# ╔═╡ c2698512-1219-4710-bdc3-d0e113497ed6
animation_slow = @animate for frame_index in 1:number_of_frames
    time_value = solution_slow.t[frame_index]
    angle_theta = solution_slow.u[frame_index][1]

    phi = rotation_rate_slow * time_value
    radial_distance = frame_offset_w1 + pendulum_length_L * sin(angle_theta)

    mass_x = radial_distance * cos(phi)
    mass_y = radial_distance * sin(phi)
    mass_z = frame_height_h1 - pendulum_length_L * cos(angle_theta)

    plot3d([0.0, mass_x], [0.0, mass_y], [0.0, mass_z],
           xlim = (-0.4, 0.4), ylim = (-0.4, 0.4), zlim = (0.0, 0.4),
           xlabel = "x [m]", ylabel = "y [m]", zlabel = "z [m]",
           label = "", title = "Spinning pendulum (slow rotation)")
end

# ╔═╡ abc84b6d-66ac-4c35-ba4e-4a240c63153b
gif(animation_slow, "spinning_pendulum_slow.gif", fps = 30)

# ╔═╡ ef05c05b-298b-4646-a297-7f4007a364e1
# ╠═╡ disabled = true
#=╠═╡
problem_slow = ODEProblem(spinning_pendulum_rhs!, initial_state, time_span, parameters_slow)
  ╠═╡ =#

# ╔═╡ e8c63e65-857e-49d3-a50d-11c5b1aa75e1
solution_slow = solve(problem_slow)

# ╔═╡ 10e453eb-c55b-4da6-b857-8a08fdd6f0a5
problem_slow = ODEProblem(spinning_pendulum_rhs!, initial_state, time_span, parameters_slow)

# ╔═╡ 3cace0a6-453b-4068-8fbd-5ca2c4fe0b68
# ╠═╡ disabled = true
#=╠═╡
solution_slow = solve(problem_slow)
  ╠═╡ =#
