...
# ╔═╡ c807d915-e8e6-45b2-89f8-cc3e297abd00
begin
...
# ╔═╡ 32c7e519-7e3e-48de-9de0-74652ca31ba3
begin
	@variables t
	@variables m g L a b Ω
	@variables θ(t)
	
	D = Differential(t)
end

# ╔═╡ d671fb04-4105-4ccf-807d-1d09bdd21dec
begin
	r = [
	    a*cos(Ω*t) + L*sin(θ)*cos(Ω*t),
	    a*sin(Ω*t) + L*sin(θ)*sin(Ω*t),
	    b - L*cos(θ)
	]
...
# ╔═╡ 6af94141-b0cd-4bc6-b4c7-6562bf8b3457
begin
...
# ╔═╡ 7088aa46-aae4-4693-b972-4c367a3c4362
begin
	T = simplify((m/2)*(dr[1]^2 + dr[2]^2 + dr[3]^2))

latexify(T)
end

# ╔═╡ 7be59e23-dbb7-498b-9d44-6d3d9d21197c
begin
	V = simplify(m*g*r[3])
...
# ╔═╡ dfe74f50-5b4a-41e3-9628-f54125b4dc5c
begin
	Lag = simplify(T - V)
...
# ╔═╡ 0115aa02-7caf-45f7-87d5-970e63e8cc53
begin
	dLdθ = simplify(Symbolics.derivative(Lag, θ))

latexify(dLdθ)
end

# ╔═╡ a568672b-7249-4b58-bb51-d812571470da
begin
	θdot = Differential(t)(θ)

dLdθdot = simplify(Symbolics.derivative(Lag, θdot))

latexify(dLdθdot)
end

# ╔═╡ 2c64663d-831e-4cc3-9b8a-bd837e6ea696
begin
	d_dt_dLdθdot = simplify(expand_derivatives(Differential(t)(dLdθdot)))

latexify(d_dt_dLdθdot)
end

# ╔═╡ 6978ecd9-3708-4558-87f6-2795a656542e
begin
	eom = simplify(d_dt_dLdθdot - dLdθ)

latexify(eom ~ 0)
end

# ╔═╡ af59fe8f-8630-49b8-8142-b7d193ffb396
begin
	θddot = Differential(t)(θdot)

θdd_solution = Symbolics.solve_for(eom ~ 0, θddot)

latexify(θddot ~ θdd_solution)
end

# ╔═╡ 0918c8a0-37b2-4bcd-b3bc-e0695a37d816
begin
    # fresh symbols for numeric evaluation (avoid conflict with θ(t))
    @variables θv ωv

    # replace θ(t) -> θv and θ̇(t) -> ωv
    θdd_expr_num = substitute(θdd_solution, Dict(θ => θv, θdot => ωv))

    # build a fast numeric function:
    # θdd_numeric(t, θv, ωv, m, g, L, a, b, Ω) -> θ̈
    f = build_function(θdd_expr_num, t, θv, ωv, m, g, L, a, b, Ω; expression=Val(false))
    θdd_numeric = eval(f)
end

# ╔═╡ 856771fe-d247-4831-960d-217eafd1b60d
begin
    function pendulum_ode!(du, u, p, t)
        m_val, g_val, L_val, a_val, b_val, Ω_val = p

        θ_val = u[1]
        ω_val = u[2]

        du[1] = ω_val
        du[2] = θdd_numeric(t, θ_val, ω_val, m_val, g_val, L_val, a_val, b_val, Ω_val)

        return nothing
    end
end

# ╔═╡ 4a1f5e8c-63b2-41f9-9d9b-054f014966b8
begin
    # parameters
    m_val = 0.1
    g_val = 9.81
...
# ╔═╡ 845de2e6-c049-4ccf-b187-d6122b3a3a29
begin
    Ω_slow = 0.6
    p_slow = (m_val, g_val, L_val, a_val, b_val, Ω_slow)

    prob_slow = ODEProblem(pendulum_ode!, u0, tspan, p_slow)
    sol_slow = solve(prob_slow, Tsit5(); reltol=1e-9, abstol=1e-9)

    sol_slow
end

# ╔═╡ 179f41da-7eff-4a70-aa35-46f571dd8733
begin
    Ω_fast = 6.0
    p_fast = (m_val, g_val, L_val, a_val, b_val, Ω_fast)

    prob_fast = ODEProblem(pendulum_ode!, u0, tspan, p_fast)
    sol_fast = solve(prob_fast, Tsit5(); reltol=1e-9, abstol=1e-9)

    sol_fast
end

# ╔═╡ b3928016-4c6c-433c-b89a-768624a3544d
begin
    plot(sol_slow.t, getindex.(sol_slow.u, 1),
        xlabel="t (s)", ylabel="θ (rad)",
        label="slow Ω=$(Ω_slow)", title="θ(t): slow vs fast"
    )
    plot!(sol_fast.t, getindex.(sol_fast.u, 1),
        label="fast Ω=$(Ω_fast)"
    )
end

# ╔═╡ 53c86e71-0d07-496d-b600-30cee8f97a0a
begin
    function pivot_position(t, a_val, b_val, Ω_val)
        return a_val*cos(Ω_val*t), a_val*sin(Ω_val*t), b_val
    end

    function bob_position(t, θ_val, L_val, a_val, b_val, Ω_val)
        x = a_val*cos(Ω_val*t) + L_val*sin(θ_val)*cos(Ω_val*t)
        y = a_val*sin(Ω_val*t) + L_val*sin(θ_val)*sin(Ω_val*t)
        z = b_val - L_val*cos(θ_val)
        return x, y, z
    end
end

# ╔═╡ a8740b52-2ad1-44f5-b100-519970d5632a
begin
    xs, ys, zs = Float64[], Float64[], Float64[]

    for (tt, uu) in zip(sol_slow.t, sol_slow.u)
        x, y, z = bob_position(tt, uu[1], L_val, a_val, b_val, Ω_slow)
        push!(xs, x); push!(ys, y); push!(zs, z)
    end

    plot(xs, ys, zs, st=:path3d,
        xlabel="x", ylabel="y", zlabel="z",
        title="Bob trajectory (slow Ω=$(Ω_slow))", label=""
    )
end

# ╔═╡ 2d49c506-a162-4b98-ade9-cc04cd1da278
begin
    anim = @animate for tt in range(tspan[1], tspan[2], length=240)
        θ_val = sol_slow(tt)[1]

        px, py, pz = pivot_position(tt, a_val, b_val, Ω_slow)
        bx, by, bz = bob_position(tt, θ_val, L_val, a_val, b_val, Ω_slow)

        Rmax = a_val + L_val + 0.2
        zmin = b_val - L_val - 0.2
        zmax = b_val + L_val + 0.2

        plot([px, bx], [py, by], [pz, bz],
            st=:path3d, lw=3,
            xlim=(-Rmax, Rmax), ylim=(-Rmax, Rmax), zlim=(zmin, zmax),
            xlabel="x", ylabel="y", zlabel="z",
            title="Spinning pendulum (slow)  t=$(round(tt,digits=2))",
            label=""
        )
        scatter!([px], [py], [pz], markersize=4, label="")
        scatter!([bx], [by], [bz], markersize=6, label="")
    end

    gif(anim, "spinning_pendulum_slow.gif", fps=20)
end
